...
from app.core.ai.gemini_client import GeminiClient, unwrap_ai_error
...
class JobService:
    ...
    async def create_job(db: AsyncSession, user_id: int, job_in: JobCreate):
        return await JobRepo.create_job(db, job_in, user_id)
    ...
    @staticmethod
    async def update_job(db: AsyncSession, job_id: int, job_update: JobUpdate):
        job = await JobService.get_job(db, job_id)
        return await JobRepo.update_job(db, job, job_update)
    ...
    async def create_version(db: AsyncSession, job_id: int, version_in: JobVersionCreate):
        ...
        if version_in.criteria_mode in ["ai", "hybrid"] and version_in.raw_jd_text:
            try:
                ...
                ai_proposals = await GeminiClient.run_jd_agent(version_in.raw_jd_text)
    ...
    async def generate_criteria_from_jd(db: AsyncSession, job_id: int, version_id: int):
        ...
        async with lock:
            try:
                ...
                ai_proposals = await GeminiClient.run_jd_agent(version.raw_jd_text)
                print(f"DEBUG: AI returned {len(ai_proposals)} proposals")
            except Exception as e:
                root_error = unwrap_ai_error(e)
