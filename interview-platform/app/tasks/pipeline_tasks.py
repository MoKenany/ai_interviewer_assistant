...
from app.core.ai.llama_client import LlamaClient, unwrap_ai_error, tokens_tracker
from app.core.ai.whisper_client import WhisperClient
...
async def _run_interview_pipeline_async(session_id: int) -> dict:
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            existing_run = existing_run_query.scalars().first()
            
            if existing_run:
                # Use existing run registered by API, just transition to running
                run = existing_run
                run.status = PipelineRunStatusEnum.running
                await db.commit()
                await db.refresh(run)
            else:
                # Fallback: create run if API didn't register it
                run = AIPipelineRun(
            ...
            try:
                ...
                if transcript:
                    logger.info(f"Pipeline {session_id} - Transcript already exists. Skipping Audio Extract and STT steps.")
                    # Mark steps as success
                    steps_dict[StepNameEnum.audio_extract].status = StepStatusEnum.success
                    steps_dict[StepNameEnum.audio_extract].completed_at = datetime.utcnow()
                    steps_dict[StepNameEnum.audio_extract].latency_ms = 0
                    steps_dict[StepNameEnum.stt].status = StepStatusEnum.success
                    steps_dict[StepNameEnum.stt].completed_at = datetime.utcnow()
                    steps_dict[StepNameEnum.stt].latency_ms = 0
                    await db.commit()
                else:
                    # Load media file
...
async def _step_audio_extract(db, session, media, steps_dict, session_id) -> str:
    ...
    try:
        ...
        is_audio = media.file_type in ["mp3", "wav", "m4a", "ogg"]
        ...
        if is_text:
            ...
            # Skip extraction for native audio files
            end = datetime.utcnow()
            step.status = StepStatusEnum.success
            step.completed_at = end
            step.latency_ms = int((end - start).total_seconds() * 1000)
            ...
            # Extract audio from video to reduce size before uploading to API
    ...
    try:
        ...
        if media.file_type == "txt":
            ...
            transcript = await asyncio.wait_for(
                WhisperClient.transcribe(audio_path),
        ...
        # ── Auto-delete media after successful transcription ────────────
        # Only deletes if the user opted in (auto_delete_media=True).
        # Best-effort: errors are logged but swallowed here to not block the pipeline.
        try:
            from app.services.session_service import SessionService
            await SessionService.auto_delete_after_transcription(db, session_id)
        except Exception as e:
            logger.error(f"Failed to auto-delete media for session {session_id}: {e}", exc_info=True)
        # ───────────────────────────────────────────────────────────────
