...
async def upload_media(
    ...
    - **auto_delete_media**: When `true`, the media file is permanently deleted
      from storage immediately after a successful transcription. If an error
      occurs, the file is preserved so you can retry. Defaults to `false`.
    ...
    logger.info(f"[API] Before queueing pipeline task - session {session_id}")
...
@router.post("/{session_id}/upload-url", response_model=SessionUploadResponse)
async def upload_media_from_url(
    session_id: int,
    url: str,
    auto_delete_media: bool = True,
    db: AsyncSession = Depends(get_db)
):
    """
    Download media from a URL and queue for pipeline processing.
    Supports YouTube, Google Drive, Vimeo, Dailymotion, TikTok, direct file links, and 1000+ sites.

    - **url**: Public media URL to download from.
    - **auto_delete_media**: Delete the downloaded file after successful transcription (default: true).
    """
    session = await db.get(InterviewSession, session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    if session.pipeline_status == PipelineStatusEnum.running:
        raise HTTPException(status_code=409, detail={"error_type": "pipeline_already_running", "message": "A pipeline is already running for this session"})
    if session.pipeline_status == PipelineStatusEnum.completed:
        raise HTTPException(status_code=409, detail={"error_type": "pipeline_already_completed", "message": "This session already has completed pipeline results."})
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={"error_type": "pipeline_queue_unavailable", "message": "Pipeline queue is unavailable.", "details": {"error": str(exc)}}
        ) from exc
    ...
    return SessionUploadResponse(
        ...
        task_id=task.id,
        status="queued",
        message="URL download complete — pipeline queued successfully"
    )
