...
def _render_evaluation_report_html(evaluation, lang: str = 'en') -> str:
    is_ar = lang == 'ar'
...
async def get_evaluation_report(evaluation_id: int, lang: str = 'en', db: AsyncSession = Depends(get_db)):
    evaluation = await EvaluationService.get_evaluation(db, evaluation_id)
    return HTMLResponse(content=_render_evaluation_report_html(evaluation, lang))
